...
CGFloat const velocityDirection = 100;
...
- (void)handlePan:(UIPanGestureRecognizer *)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        if (velocityForSelectedDirection > velocityDirection
            && (self.direction == ZFModalTransitonDirectionRight
                || self.direction == ZFModalTransitonDirectionBottom)) {
                [self finishInteractiveTransition];
            } else if (velocityForSelectedDirection < -velocityDirection && self.direction == ZFModalTransitonDirectionLeft) {
                [self finishInteractiveTransition];
            } else {
...
- (void)updateInteractiveTransition:(CGFloat)percentComplete
{
    ...
    CGRect updateRect;
    if (self.direction == ZFModalTransitonDirectionBottom) {
        CGFloat moveY = CGRectGetHeight(fromViewController.view.bounds) * percentComplete;
        //        if (moveY < 0) {
